...
<div id="wrapper">
	...
	<div id="main">
		...
		<div id="content">
		<noscript>
		<div class="noscriptalert">現在お使いのブラウザのJavaScriptの設定が無効になっています。<br/>すべての機能を利用するためには、JavaScriptの設定を有効にしてください。</div>
		</noscript> 
			...
			<% } }%>
			<p class="topic">ホーム</p>
			<p><%= userName + " " + userAccountId %> さん、こんにちは。</p>
			<p>左のメニューから項目をお選び下さい。</p>
			<p class="topic">掲示板</p>
			
			<div class="button_align2">
				<a href="/bbs/entry">
					<span class="entry_button"><img align="top" alt="新規登録" src="resorce/bbsentry.png" width="25px">新規投稿する</span>
				</a>
			</div>
			...
			<% } else {%>
				<div class="con_topic">
					...
					<% for (Contribution conTopic : contributions.values()) { %>
						<tr>
							<td class="topic_img" rowspan="2" align="center">
								<img alt="ユーザ" src="resorce/user.png" width="25px">
							</td>
							<th colspan="2">
								<a href="#contribution<%= conTopic.getConId() %>"><%= conTopic.getConTitle()%> (<%=conTopic.getCommNum() %>)</a>
							</th>
						</tr>
						<tr>
							<td class="con_topic_username">
								<%= conTopic.getConUserName() + "(" + conTopic.getConUserAccountId() %>) さん
							</td>
							<td class="con_topic_released" width="180px">
								<%= conTopic.getConReleased() %>
							</td>
						</tr>
					<% } %>
					</table>
			...
			<div class="bbs">
				<table class="bbs_table">
					<tr>
						<td class="bbs_img" rowspan="4" valign="top">
							<img alt="ユーザ" src="resorce/user.png" width="30px">
						</td>
						<th class="bbs_th">
							投稿者：
						</th>
						<td colspan="3">
							 <%= con.getConUserName() + "(" + con.getConUserAccountId() %>) さん
						</td>
					</tr>
					<tr>
						<th class="bbs_th">
					...
					<tr>
						<td class="bbs_content" colspan="4">
							<pre><% out.print(con.getConText()); %></pre>
						</td>
					</tr>
				</table>
				<div class="button_align_comment">
					<p class="comment_num"><%=con.getCommNum() %>件のコメント</p>
					<p id="viewcommentbtn<%= con.getConId() %>" class="viewcommentbtn">表示</p></div>
				<div id="comment<%= con.getConId() %>" class="comment">
					<div class="comment_form">
						...
						<form name="form" action="Comment" method="POST" onsubmit="return commentCheck(this.text, this.conId)">
							<img class="commentuser_img"alt="コメントユーザ" src="resorce/commentuser.png" width="30px" align="middle">
							<div id="alert_place<%=con.getConId() %>"></div>
							<textarea class="textareafeild" placeholder="この書き込みに対し500文字以内でコメントする" name="text" maxlength="500"></textarea>
					...
					%>
						...
						<table class="comment_table">
							<tr>
								<td class="bbs_img" rowspan="2">
									<img class="commentuser_img"alt="コメントユーザ" src="resorce/commentuser.png" width="30px" align="middle">
								</td>
								<td class="comment_name">
									<%= com.getCommUserName() + "(" + com.getCommUserAccountId() %>) <font color="#747474">さん</font>
								</td>
